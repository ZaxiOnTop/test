...
(function () {
  ...
  function injectPremiumGate(label) {
    ...
    var css = ''
      + '.amf-gate{position:absolute;inset:0;z-index:60;display:flex;flex-direction:column;overflow-y:auto;'
      + 'padding:24px;background:rgba(243,247,252,.72);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px)}'
      + '.amf-gate-card{max-width:420px;width:100%;margin:auto;background:#fff;border:1px solid #e7ebf2;border-radius:20px;'
  ...
  function openSubscribeModal() {
    ...
    function close() { document.body.style.overflow = _prevOverflow; veil.remove(); }
  ...
  function ensureSubCss() {
    ...
    var css = ''
      + '.amf-sub-veil{position:fixed;inset:0;z-index:9999;overflow-y:auto;-webkit-overflow-scrolling:touch;'
      + 'display:flex;flex-direction:column;padding:18px;background:rgba(6,20,48,.55);backdrop-filter:blur(3px);-webkit-backdrop-filter:blur(3px)}'
      + '.amf-sub{position:relative;margin:auto;width:100%;max-width:620px;background:#fff;border-radius:22px;'
      + 'padding:28px clamp(18px,3vw,34px) 24px;box-shadow:0 40px 90px -30px rgba(0,30,90,.55);font-family:inherit}'
      + '.amf-sub-x{position:absolute;top:14px;right:14px;width:36px;height:36px;border-radius:50%;border:none;background:#f3f7fc;'
      + 'color:#5a6475;font-size:22px;line-height:1;cursor:pointer;z-index:2}'
      + '.amf-sub-x:hover{background:#e7ecf5}'
      + '.amf-sub h3{font-size:clamp(18px,5vw,22px);font-weight:700;color:#013a87;letter-spacing:-.02em;margin:0 44px 6px 0}'
      + '.amf-sub-lead{font-size:13.5px;color:#6b7280;margin-bottom:18px}'
      + '.amf-plans{display:grid;grid-template-columns:1fr 1fr;gap:12px}'
      + '.amf-plan{position:relative;border:1.5px solid #e7ebf2;border-radius:16px;padding:18px 15px 15px;display:flex;flex-direction:column}'
      ...
      + '.amf-pp{font-weight:800;font-size:24px;color:#0f1622;margin:6px 0 12px}'
      ...
      + '@media(max-width:560px){.amf-plans{grid-template-columns:1fr}.amf-sub{border-radius:18px;padding:24px 18px 20px}'
      + '.amf-ribbon{left:auto;right:14px;transform:none}}';
    var st = document.createElement('style'); st.textContent = css; document.head.appendChild(st);
  }
})();
